      ...
      uint8_t * packet)
{
  ...
  if (req!=NULL){
    ...
    for(packet_tmp = req->packets; packet_tmp != NULL; packet_tmp = packet_tmp->next){
      ...
      if (packet_tmp->mapping != NULL) {
        rewrite_ip_packet_to_forward_for_nat(sr, buf, packet_tmp->in_iface, packet_tmp->mapping);
      }
...
struct sr_arpreq *sr_arpcache_queuereq(struct sr_arpcache *cache,
                                       ...
                                       char *iface)
{
  return sr_arpcache_queuereq_with_nat(cache, ip, packet, packet_len, iface, NULL, NULL);
}

struct sr_arpreq *sr_arpcache_queuereq_with_nat(struct sr_arpcache *cache,
                                       uint32_t ip,
                                       uint8_t *packet,           /* borrowed */
                                       unsigned int packet_len,
                                       char *iface,
                                       ...
                                       struct sr_nat_mapping *mapping)
{
	...
	new_pkt->in_iface = (char *)malloc(sr_IFACE_NAMELEN);
        ...
        if (NULL != iface) strncpy(new_pkt->iface, iface, sr_IFACE_NAMELEN);
        if (NULL != in_iface) strncpy(new_pkt->in_iface, in_iface, sr_IFACE_NAMELEN);
        if (in_iface == NULL) new_pkt->in_iface = NULL;
        new_pkt->mapping = mapping;
    ...
    if (entry) {
        ...
        for (pkt = entry->packets; pkt; pkt = nxt) {
            ...
            if (pkt->in_iface)
                free(pkt->in_iface);
            if (pkt->mapping)
                sr_nat_destroy_mapping_helper(pkt->mapping);
            free(pkt);
        }
