...
const char *internal_if;
const char *external_if;
/*-----------------------------------------------------------------------------
 *---------------------------------------------------------------------------*/
...
int main(int argc, char **argv)
{
    ...
    uint32_t icmp_query_timeout = 60;
    uint32_t tcp_estab_idle_timeout = 7440;
    uint32_t tcp_trans_idle_timeout = 300;
    ...
    while ((c = getopt(argc, argv, "hs:v:p:u:t:r:l:T:I:E:R:n")) != EOF)
    {
        switch (c)
        {
            ...
            case 'I':
                ...
                break;
            case 'E':
                tcp_estab_idle_timeout = atoi((char *) optarg);
                break;
            case 'R':
    ...
    if (use_nat) {
      ...
      sr_nat_init(sr.nat, icmp_query_timeout, tcp_estab_idle_timeout, tcp_trans_idle_timeout);
      sr.nat->sr = &sr;
    }
