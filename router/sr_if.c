...
{ 
  return (strcmp(interface, internal_if) == 0) ? true : false; 
}

bool is_external_if(char *interface) 
{ 
  return (strcmp(interface, external_if) == 0) ? true : false; 
}

bool sr_contains_ip(struct sr_instance *sr, uint32_t ip) {
  struct sr_if* if_walker = sr->if_list;
  while(if_walker) {
    if (ip == if_walker->ip) return true;
...
char* sr_get_if_name(struct sr_instance* sr, const unsigned char *eth_addr)
{
    ...
    while(if_walker)
    {
       /* There is no difference between signed and unsigned char. Actually on many platforms unqualified char is signed. */
       if(!strncmp((const char*)if_walker->addr, (const char *)eth_addr, ETHER_ADDR_LEN))
...
/* Returned address should in NBO */ 
